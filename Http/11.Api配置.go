...
type User struct {
	...
	Password string `json:"-"  xml:"-"` //我们需要把Password忽略掉，不能将密码传给API哟！
...
func Users(w http.ResponseWriter,r *http.Request)  { //定义一个API，我们可以用Python语言和其交互。可参考本地文件："/yinzhengjie/python/client.py"
	r.ParseForm()
	format := r.FormValue("yinzhengjie") //定义关键字用来选择输出格式如："http://192.168.16.109:8088/users?yinzhengjie=xml"
	var users []User
	var resp  Response
	...
	switch format {
	case "json":
		w.Header().Set("Content-Type","application/json") //可以给用户格式化输出，让用户更加容易读取信息.
		buf,_ = json.Marshal(&resp)
	case "xml":
		w.Header().Set("Content-Type","text/xml") //可以给用户格式化输出，让用户更加容易读取信息.
...
func main() {
	...
	/*
	扩展小知识：
		http.HandlerFunc ----> 它是一个函数，通过http.HandleFunc进行挂载。
		http.Handle--->它是一个接口，通过http.Handler进行挂载。
		如果将Login函数转换成http.Handler呢？其实很简单，通过	http.HandlerFunc(Login)就可以将其转换成http.Handler啦！
	*/

	loginCounter := NewCounter(http.HandlerFunc(Login))
	http.Handle("/login",loginCounter)
	http.HandleFunc("/loginCounter",loginCounter.GetCounter) //通过HandleFunc绑定路由信息。
	...
	http.Handle("/Http/static/",http.FileServer(http.Dir("."))) //用"http.FileServer(http.Dir("."))"方法指定"/Http/static/"为根目录。
	h := handlers.LoggingHandler(os.Stderr,http.DefaultServeMux) //定义一个有日志输出的对象。它其实也是一个中间件（装饰器）。将日志交给LoggingHandler处理后在传给默认路由"DefaultServeMux"
	c := NewCounter(h)
	log.Fatal(http.ListenAndServe(":1234",c)) //启动一个http服务使其监听本地所有的8080端口。并将访问记录传给h，你可以理解所有的流量都要经过h。
}
